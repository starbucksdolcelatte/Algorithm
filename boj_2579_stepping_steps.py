...
f = open("input.txt", 'r')
N = int(f.readline())
step = [int(f.readline()) for _ in range(N)]
f.close()
#print(step)
'''

sum = [[0 for _ in range(N)] for _ in range(2)]
sum[0][0] = step[0] # 1칸을 올라 첫 번째 계단을 밟을 때
sum[0][1] = sum[0][0]+ step[1] # 연속 2번 1칸을 올라 두 번째 계단을 밟을 때
sum[1][1] = step[1] # 2칸을 올라 두 번째 계단을 밟을 때
#print(sum)

for i in range(2,N):
    sum[0][i] = sum[1][i-1] + step[i] # 1칸 오를 때. 2칸 전으로부터 와야 한다.
    sum[1][i] = max(sum[0][i-2], sum[1][i-2]) + step[i] # 2칸 오를 때. 1칸 전으로부터 와야 한다.
#print(sum)
print(max(sum[0][-1], sum[1][-1]))
